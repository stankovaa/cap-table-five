...
y1_step = float(y1_tickvals[1] - y1_tickvals[0]) if len(y1_tickvals) > 1 else 10
y1_range = [y1_tickvals[0] - y1_step * 0.5, y1_tickvals[-1] + y1_step * 0.5]
...
y2_range = [y2_tickvals[0] - y2_step * 0.5, y2_tickvals[-1] + y2_step * 0.5]
# ymin_m, ymax_m = -20,60
# dtick=20
...
disclaimer_wrapped = "<br>".join(textwrap.wrap(disclaimer, width=27))
CHART_H = 450
...
fig1.update_layout(
    ...
    legend=dict(
        ...
        x=1.02, xanchor="left",
        y=1.0,  yanchor="top",
        entrywidth=20, entrywidthmode="pixels",
    ),
    annotations=[
        dict(
    ...
    margin=dict(t=80, r=160, b=40, l=70)
)
